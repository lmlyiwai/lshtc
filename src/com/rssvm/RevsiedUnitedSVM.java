...
public class RevsiedUnitedSVM {
	...
	public RevsiedUnitedSVM(Structure structure, Parameter param, Problem prob, int maxIteration, double precision) {
		this.structure = structure;
		this.param = param;
		this.prob = prob;
		this.maxIteration = maxIteration;
		this.weights = new double[this.structure.getAllNodes().length][this.prob.n];
		this.labels = this.structure.getLeaves();
	...
	public double[] getNodeWeight(int id, Problem prob) {
		...
		double scale = 1 / (2 * (double)des.length);
	...
	public void updateNode(Problem prob, Parameter param, int id) {
		...
		for(int i = 0; i < nodes.length; i++) {
			...
			lambda[i] = this.structure.getDes(nodes[i]).length;
		}
	...
	public double getNodeNormal(int id, Problem prob) {
		...
		double scale = 1 / (4 * (double)des.length);
	...
	public double getPrimalObj(int[] nodes, Problem prob, double c) {
		...
		for(i = 0; i < nodes.length; i++) {
			id = nodes[i];
			lambda = this.structure.getDes(id).length;
