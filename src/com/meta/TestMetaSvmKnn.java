...
import com.stack.SVMKNN;
import com.tools.Statistics;

public class TestMetaSvmKnn {
...
//		svmknn.train(train, param, 5, k1, c, k2);
		...
		double[][] trainpv = svmknn.predictValues(w, train.x, train.n);
		svmknn.scale(trainpv);
		
		double[][] testpv = svmknn.predictValues(w, test.x, test.n);
		svmknn.scale(testpv);
		
		int[][] pre = svmknn.predictNear(trainpv, testpv, train.y, 31);
