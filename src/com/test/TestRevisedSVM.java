...
public class TestRevisedSVM {
	...
	public void test() throws IOException, InvalidInputDataException {
		...
		Parameter param = new Parameter(10000, 1000, 0.001);
		RevisedSVM rs = new RevisedSVM(tree, prob, param);
		rs.train();
...
System.out.println("num of test samples = " + testprob.l);
		...
		int[][] pre = rs.predict(testprob.x);
		double microf1 = Measures.microf1(rs.getValidateLabels(prob.y), testprob.y, pre);
		double macrof1 = Measures.macrof1(rs.getValidateLabels(prob.y), testprob.y, pre);
